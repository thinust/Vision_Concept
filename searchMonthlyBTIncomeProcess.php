...
if (empty($date)) {
    ...
    $d_income_rs = Database::search("SELECT SUM(`payments`.`b_transfer_payments`) FROM `payments` JOIN `invoice` ON `payments`.`invoice_id`=`invoice`.`id` WHERE `invoice`.`status_id`='1' AND YEAR(`payments`.`date`) = '" . $year . "' AND MONTH(`payments`.`date`) = '" . $month . "' ORDER BY `payments`.`date` ASC");
