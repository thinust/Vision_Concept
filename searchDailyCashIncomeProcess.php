...
if (empty($date)) {
    ...
    $d_income_rs = Database::search("SELECT SUM(`payments`.`cash_payments`) FROM `payments` JOIN `invoice` ON `payments`.`invoice_id`=`invoice`.`id` WHERE `invoice`.`status_id`='1' AND  YEAR(`payments`.`date`) = '" . $year . "' AND MONTH(`payments`.`date`) = '" . $month . "' AND DAY(`payments`.`date`) = '" . $day . "' ORDER BY `payments`.`date` ASC");
        ...
        <div class="card itemcard " aria-hidden="true">
            ...
            <div class="card-body text-center">
                <div class="mt-5 mb-4">
                    ...
                    <span class="col-12">Rs. <?php echo $d_income_data["SUM(`payments`.`cash_payments`)"]; ?> .00</span><br>
                </div>
